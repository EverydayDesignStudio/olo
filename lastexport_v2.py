...
def get_options(parser):
    ...
    if options.infotype == "loved":  #in case we wanted to grab the 'loved' tracks; note: the timestamps don't work for them
        infotype = "lovedtracks"
    elif options.infotype == "banned": #we likely do not need 'banned' tracks, but it is possible to grab them with the LastFM API
        infotype = "bannedtracks"
    else:
        infotype = "recenttracks"  #grab only last 50 songs (i.e., first 'page')
...
def connect_server(server, username, startpage, sleep_func=time.sleep, tracktype='recenttracks'):
    ...
    #**this interval exception does not always trigger when connectivity is lost. 
    #**I think it has to do with where in the process the script is when the connection is lost.
    #**it also has trouble re-starting the archive download when the pause is longer than 60 seconds
    for interval in (1, 5, 10, 60, 100):  
        try:
        ...
        raise last_exc
        #**could include a statement re-starting the process if it has failed
        
    response = f.read()
    f.close()

    #bad hack to fix bad xml (may not need for our purposes)
    response = re.sub('\xef\xbf\xbe', '', response)
    return response

def get_pageinfo(response, tracktype='recenttracks'):
    """Check how many pages of tracks the user has."""
        ...
        for page, totalpages, tracks in get_tracks(server, username, startpage, tracktype=infotype):
            ...
            for track in tracks:
                if infotype == 'recenttracks':
                    ...
                    #Can not use timestamp as key for loved/banned tracks as it's not unique (but we don't need them)
                    n += 1
